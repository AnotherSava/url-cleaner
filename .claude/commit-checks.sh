...
# nothing else reads a change before it lands; CI runs after the push, when a break is already public.
#
# Runs the conventions checker, then the same restore, build and test as the build job in
# .github/workflows/build.yml, in the same order. Keep the two level: a gate that differs from CI
# disagrees with it about what main requires. The tag-only publish and packaging steps are not here.
#
# Prerequisites are `python3` and `dotnet` on PATH. Their absence fails rather than skips: a check
